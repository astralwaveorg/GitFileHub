...
[![PM2](https://img.shields.io/badge/PM2-Managed-2B037A)](https://pm2.keymetrics.io/)

通过简洁的 Web 界面管理多个 Git 仓库，支持文件浏览、语法高亮预览、在线编辑、拖拽上传、一键下载。所有变更自动提交推送到远程仓库，提供类似 GitHub Raw 的文件直链能力。支持 GitHub / Gitea / Gitee 三大平台的 Webhook 自动同步与定时拉取，无需数据库服务，一行命令即可部署。
...
| ⚡ **一键部署** | 内置部署脚本，一行命令自动安装 Node.js、PM2、初始化数据库并启动服务 |
...
这是最简单的部署方式，适合绝大多数场景。部署脚本会自动安装 Node.js、PM2、初始化数据库并启动服务，全程无需手动操作。

**前置要求：**
- 一台运行 Ubuntu / Debian / CentOS / Rocky Linux 的服务器
- root 或 sudo 权限

**部署步骤：**

```bash
# 一行命令，从 GitHub 远程部署到 /opt/gitfiledock
...
2. 安装 PM2 进程管理器
3. 安装项目依赖
4. 生成 `.env` 配置文件（含随机 JWT 和加密密钥）
5. 初始化数据库并创建默认管理员
6. 构建 Next.js 生产版本
7. 通过 PM2 启动服务并配置开机自启

启动后访问 `http://your-server:3000`，使用默认管理员账号登录：**admin** / **admin123**

> 提示：部署完成后请立即在「设置 → 个人信息」中修改默认密码。

### 手动部署

如果你已有 Node.js 环境或需要自定义配置，也可以手动部署。
...
使用 `manage.sh` 脚本管理服务，支持停止、重启、查看日志、查看状态和卸载操作：
...
# 重启服务
pm2 restart gitfiledock

# 停止服务
pm2 stop gitfiledock
```

### 更新版本

使用内置的更新脚本，一键拉取最新代码、更新依赖、重建并重启服务：

```bash
curl -fsSL https://raw.githubusercontent.com/astralwaveorg/GitFileHub/main/update.sh | bash
```

或在项目目录下手动更新：

```bash
cd /opt/gitfiledock   # 默认安装目录
git pull
npm install
npx prisma generate
npx prisma db push --skip-generate
...
数据库文件默认位于 `data/gitfiledock.db`。Git 仓库本地存储在 `data/repos/` 目录下。所有数据持久化存储在本地文件系统中，更新版本不会丢失数据。
...
| [PM2](https://pm2.keymetrics.io/) | 最新 | 进程管理 | 守护进程、自动重启、日志管理、开机自启 |
...
├── deploy.sh                        # 一键部署脚本（自动安装依赖、初始化数据库、构建启动）
├── manage.sh                        # 服务管理脚本（停止/重启/日志/状态/卸载）
├── update.sh                        # 版本更新脚本（拉取代码、更新依赖、重建重启）
...
更新会自动拉取最新代码、更新依赖、重建并重启服务，数据库和仓库数据不会丢失。

### Q: 如何备份和迁移数据？

```bash
# 备份数据（数据库 + 仓库文件）
cp -r /opt/gitfiledock/data ./backup-$(date +%Y%m%d)

# 迁移到新服务器
scp -r ./backup-20260417 new-server:/opt/gitfiledock/data/
# 在新服务器上重启服务
pm2 restart gitfiledock
```
