...
### 模块 6：文件编辑
- [ ] 文本/代码文件可编辑（CodeMirror 6）
- [ ] 编辑器：行号、语法高亮、Tab 缩进
- [ ] 图片和二进制文件不显示编辑按钮
- [ ] 保存时自动 git add → commit → push
- [ ] 提交状态实时显示
...
### 模块 10：复制链接
- [ ] 复制文件直链（类似 GitHub Raw）
- [ ] 复制目录链接（浏览器中浏览目录）
- [ ] 自动获取当前域名生成链接
- [ ] 链接支持 wget/curl 直接下载文件内容
- [ ] 链接支持匿名访问（即使文件在隐藏列表中）
...
- [ ] 同步进度显示
- [ ] 冲突提示
- [ ] 定时同步（30min/1h/2h/4h/关闭），使用 node-cron
- [ ] Webhook 自动同步
...
- [x] 生成 SSH Deploy Key
- [x] 编写 PLAN.md
- [ ] 创建 LICENSE / .gitignore / README.md
- [ ] 初始化 Next.js 项目
- [ ] 配置 Prisma + SQLite
...
- [ ] 仓库 CRUD API
- [ ] 设置页面（仓库管理、密钥管理）
- [ ] Git 操作封装（clone/pull/commit/push）
...
- [ ] 复制链接按钮
- [ ] /raw/[repo]/[...path] 端点
- [ ] wget/curl 兼容（Content-Disposition）
